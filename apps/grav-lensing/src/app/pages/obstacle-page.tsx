...
export const ObstaclePage = () => {
  ...
  const handleNext = useCallback(() => {
    amplitude.getInstance().logEvent('Click on Next Button',{'CurrentPage':'Obstacle Blocks Light'})
  ...
  useEffect(() => {
    ...
    if (offset > obstacleRadius) {
      ...
      setLineLength(`calc(${obstacleCentre[0]} - ${galaxyPos[0]} - ${Math.sqrt(Math.pow(obstacleRadius, 2) - Math.pow(offset, 2))}px)`)
    }
  }, [obstaclePos])
