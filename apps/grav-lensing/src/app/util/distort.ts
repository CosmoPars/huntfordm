 ...
 */
export const getLensEffectData = (darkMatter: DarkMatter[], position: [x: number, y: number]): LensEffectData[] => {
  return darkMatter.map<LensEffectData>((dm) => {
    ...
    const vector = [dm.position[0] - position[0], -dm.position[1] + position[1]]
 ...
 */
export const getTransformMatrixForDistortion = (lenses: LensEffectData[]) :
TransformMatrix => {
  ...
  const eigencomponent = Math.sqrt(Math.pow(m11 + m22, 2) - 4 * (m11 * m22 - Math.pow(m12, 2)))
  ...
  if (eigenval1 - eigenval2 < 1e-10) {
    if (Math.abs(m11 - m22) >= 1e-10) {
      eigenvec1x = -m12 / Math.sqrt(Math.pow(m12, 2) + Math.pow(m22 - eigenval1, 2))
      eigenvec1y = 0.5 * (m11 - m22) / Math.sqrt(Math.pow(m12, 2) + 0.25 * Math.pow(m11 - m22, 2))
    }
  } else {
    if (Math.abs(m11 - eigenval1) < 1e-10) {
      const divisor = Math.sqrt(Math.pow(m12, 2) + Math.pow(m22 - eigenval1, 2))
      eigenvec1x = (m22 - eigenval1) / divisor
      eigenvec1y = -m12 / divisor
    } else {
      const divisor = Math.sqrt(Math.pow(m12, 2) + Math.pow(m11 - eigenval1, 2))
  ...
  const newM11 = eigenval1 * Math.pow(Math.cos(rotateAngle), 2) + (1 / eigenval1) * Math.pow(Math.sin(rotateAngle), 2)
  const newM12 = (eigenval1 - 1 / eigenval1) * Math.cos(rotateAngle) * Math.sin(rotateAngle)
  const newM22 = eigenval1 * Math.pow(Math.sin(rotateAngle), 2) + (1/eigenval1) * Math.pow(Math.cos(rotateAngle), 2)
