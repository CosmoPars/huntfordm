...
export type GridProps = {
  scanGridCallback: (position:[number,number], existingResult?: ScanResult) => ScanResult
...
export const GridContainer = styled.div<LayoutProps>`
  ...
  grid-template-columns: repeat(${props => props.columns}, ${props=>100/props.columns}%);
  grid-template-rows: repeat(${props => props.rows}, ${props=>100/props.rows}%);
  ...
  useEffect(() => {
    ...
    return (
      <GridContainer columns = {columns} rows = {rows}>
        {buttonList.map((props, index) =>(<GridButton key={index} {...props} />))}
      </GridContainer>
    )
}
