...
const SliderRail = styled.div<SliderRailProps>`
  ...
  width: ${props => props.axis === 'y' ? props.railThickness !== undefined ? props.railThickness : 1 : props.width}px;
  height: ${props => props.axis === 'y' ? props.width : props.railThickness !== undefined ? props.railThickness : 1}px;
...
const SliderPoint = styled.div<SliderPointProps>`
  position: absolute;
  left: ${props => props.axis === 'y' ? '50%' : `${(props.initialOffset + 0.5) * 100}%`};
  top: ${props => props.axis === 'y' ? `${(props.initialOffset + 0.5) * 100}%` : '50%'};
  width: 0px;
  height: 0px;
`
...
const SliderHead = styled.div<SliderHeadProps>`
  ...
  transform: translate(-50%, -50%) ${props => props.axis === 'y' ? 'rotate(90deg)' : ''};
`
...
export const Slider = ({
  ...
  const handleDrag = useCallback((event: DraggableEvent, data: DraggableData) => {
    ...
    onValueUpdated?.(((railProps.axis === 'y' ? data.y : data.x) / railProps.width) + initialValue)
  }, [onValueUpdated, railProps, initialValue])

  return (
    <SliderRail {...railProps}>
      <Draggable axis={railProps.axis} bounds={{
      ...
      }} onDrag={handleDrag} onStop={handleDragStop} disabled={disabled}>
        <SliderPoint initialOffset={initialValue}>
          <SliderHead height={sliderHeadHeight} axis={railProps.axis}>
            <SliderHeadIcon color={railProps.railColor || '#FF0'} height={sliderHeadHeight} />
          </SliderHead>
        </SliderPoint>
      </Draggable>
    </SliderRail>
