...
export function GravitationalLensingSimulation({
  ...
  const handleResize = useCallback((newSize: [number, number]) => {
    if (simulation) {
      simulation.updateTextureScale([(1 / (textureScale * textureAspect)), (1 / textureScale)])
  ...
  useEffect(() => {
    setLenses([{position: [lensCenter[0] + lensOffset[0], lensCenter[1] + lensOffset[1]], mass: lensMass}])
  }, [lensCenter, lensOffset, lensMass])

  const simulationDraw = useCallback((size: [number, number], canvas: HTMLCanvasElement) => {
    if (!simulation) {
      const context = canvas.getContext('webgl')
      if (!context) {
        console.error('[Gravitational Lensing Simulation] Unable to get WebGL Context!')
        return
      }

      const newSim = new LensingSimulationProgram(context,
  ...
  const draw = useCallback((size: [number, number], canvas: HTMLCanvasElement) => {
    ...
    if (context) {
      ...
      context.beginPath()
      context.ellipse((lensCenter[0] * size[0]) + lensOffset[0], (lensCenter[1] * size[1]) + lensOffset[1], 20, 20, 0, 0, 2 * Math.PI)
      context.fill()
    }
  }, [lensCenter, lensOffset])

  return (
    <ResponsiveCanvas id='LensingSimulation' singleDraw={simulationDraw} onResize={handleResize} />
  )
}
